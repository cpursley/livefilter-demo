defmodule LiveFilter.UIState do
  ...
  def merge_ui_with_filters(%FilterGroup{} = filter_group, ui_state, opts \\ []) do
    ...
    %{filter_group | 
      filters: filter_group.filters ++ ui_filters
    }
  end
  ...
  def filters_to_ui_state(filters_or_group, opts \\ []) do
    filters = case filters_or_group do
    ...
    Enum.reduce(fields, base_state, fn {field, type}, state ->
      ...
      value = case Map.get(transforms, field) do
        nil -> value
  ...
  defp default_converter(ui_state, opts) do
    ...
    # Handle search query
    filters = if is_map(ui_state) && Map.get(ui_state, :search_query) do
    ...
    filters = if is_map(ui_state) && Map.get(ui_state, :selected_values) do
    ...
    # Handle date ranges
    filters = if is_map(ui_state) && Map.get(ui_state, :date_ranges) do
    ...
    filters = if is_map(ui_state) && Map.get(ui_state, :boolean_flags) do
  ...
  defp detect_type_from_value(_field, value) do
    cond do
      ...
      is_list(value) -> :enum  # Lists are typically enum selections
