defmodule LiveFilter.Components.FilterSelector do
  ...
  def update(assigns, socket) do
    ...
    inactive_filters = Enum.reject(assigns.available_filters, fn filter ->
      field = case filter do
        ...
        <.dropdown_menu id={"#{@id}-dropdown"}>
          ...
          <.dropdown_menu_content align="start" class="w-[200px]">
            ...
            <.dropdown_menu_item
              ...
              on-select={Phoenix.LiveView.JS.push("select_filter", value: %{field: elem(filter, 0)}, target: @myself)}
            >
              <.icon :if={get_filter_icon(filter)} name={get_filter_icon(filter)} class="mr-2 h-4 w-4" />
