    ...
    <div id={"#{@id}-wrapper"}>
      ...
      <.dropdown_menu_content align="start" class="w-[280px] p-0">
        <div :if={@searchable} class="p-2 pb-0">
          <div class="relative">
            <.icon name="hero-magnifying-glass" class="absolute left-2 top-2 h-4 w-4 text-muted-foreground" />
            <.input
              id={@search_id}
              type="text"
              placeholder="Search..."
        ...
        <div class="max-h-[300px] overflow-auto p-2">
          ...
          <%= if @clearable && @selected != [] do %>
            ...
            >
              <.icon name={@clear_icon} class="mr-2 h-4 w-4" />
              Clear selection
            </.dropdown_menu_item>
          <% end %>
        </div>
