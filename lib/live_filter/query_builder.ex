defmodule LiveFilter.QueryBuilder do
  ...
  defp build_dynamic(%LiveFilter.FilterGroup{filters: filters, groups: groups, conjunction: conjunction}) do
    IO.inspect(filters, label: "Building dynamics for filters")
    ...
    IO.inspect(length(all_dynamics), label: "Number of dynamics to combine")
    
    case all_dynamics do
      [] -> nil
      [single] -> single
      multiple -> 
        IO.puts("Combining #{length(multiple)} dynamics with #{conjunction}")
        combine_dynamics(multiple, conjunction)
    end
  end

  defp build_filter_dynamic(%LiveFilter.Filter{field: field, operator: operator, value: value, type: type}) do
