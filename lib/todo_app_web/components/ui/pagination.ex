defmodule TodoAppUi.Pagination do
  @moduledoc """
  ...
  * `:size` - Link size variant (default, sm, lg, icon)
  ...
  attr :rest, :global, include: ~w(phx-click phx-target phx-value-page)

  def pagination_first(assigns) do
    ~H"""
    <.pagination_link aria-label="Go to first page" {@rest} class={@class}>
  ...
  def pagination_last(assigns) do
    ~H"""
    <.pagination_link aria-label="Go to last page" {@rest} class={@class}>
