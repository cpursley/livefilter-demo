defmodule LiveFilter.EventRouterTest do
  ...
  describe "parse_filter_event/2" do
    test "parses default filter event pattern" do
      ...
      assert {:ok, :due_date, :cleared} = EventRouter.parse_filter_event("filter_due_date_cleared")
      assert {:ok, :is_active, :toggled} = EventRouter.parse_filter_event("filter_is_active_toggled")
    end
    
    test "handles multi-word field names" do
      assert {:ok, :due_date, :selected} = EventRouter.parse_filter_event("filter_due_date_selected")
      assert {:ok, :created_at, :updated} = EventRouter.parse_filter_event("filter_created_at_updated")
    end
    ...
    test "validates against field whitelist" do
      assert {:ok, :status, :changed} = 
        EventRouter.parse_filter_event("filter_status_changed", fields: [:status, :priority])
      
      assert {:error, :invalid_field} = 
        EventRouter.parse_filter_event("filter_unknown_changed", fields: [:status, :priority])
    end
  ...
  describe "route_event/3" do
    test "routes to matching handler" do
      ...
      result = EventRouter.route_event("filter_status_changed", %{}, 
        handlers: handlers,
        socket: %{}
      )
    ...
    test "calls fallback for non-matching events" do
      ...
      result = EventRouter.route_event("unknown_event", %{},
        handlers: %{},
        fallback: fallback,
        socket: %{}
      )
    ...
    test "calls fallback when no handler matches" do
      ...
      result = EventRouter.route_event("filter_priority_changed", %{},
        handlers: handlers,
        fallback: fallback,
        socket: %{}
      )
    ...
    test "returns default when no fallback and no match" do
      result = EventRouter.route_event("unknown", %{}, 
        handlers: %{},
    ...
    test "uses custom parse options" do
      ...
      result = EventRouter.route_event("quick_project_selected", %{},
        handlers: handlers,
  ...
  describe "extract_event_value/2" do
    ...
    test "prioritizes clear over other values" do
      assert {:clear, true} = EventRouter.extract_event_value(%{
        "clear" => true,
        "value" => "ignored"
      })
    end
