defmodule LiveFilter.QuickFiltersTest do
  ...
  describe "search_filter/2" do
    ...
    test "uses custom field and operator" do
      filter = QuickFilters.search_filter("john", 
        field: :author_name,
        operator: :contains
      )
  ...
  describe "multi_select_filter/3" do
    ...
    test "uses custom operator" do
      filter = QuickFilters.multi_select_filter(:tags, ["urgent", "bug"],
  ...
  describe "date_range_filter/3" do
    ...
    test "uses custom operator" do
      filter = QuickFilters.date_range_filter(:created_at, ~D[2025-01-01],
        operator: :after
      )
  ...
  describe "from_params/2" do
      ...
      definitions = [
        ...
        {"min_price", fn v -> 
          case Float.parse(v) do
            {price, _} -> QuickFilters.numeric_filter(:price, price, operator: :greater_than_or_equal)
            _ -> nil
          end
        end}
      ]
    ...
    test "handles prefix option" do
      ...
      definitions = [
        {"status", fn v -> QuickFilters.multi_select_filter(:status, v) end},
        {"urgent", fn "true" -> QuickFilters.boolean_filter(:urgent, true); _ -> nil end}
      ]
