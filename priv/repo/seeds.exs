  ...
  todos ++
    Enum.map(1..5, fn i ->
      %{
        ...
        description:
          "Critical production issue that needs immediate attention - affecting users in #{Enum.random(["payment", "authentication", "data processing", "API", "UI rendering"])}",
...
todos =
  todos ++
    Enum.map(1..10, fn i ->
      ...
      %{
        title:
          "Overdue: #{Enum.random(["Update", "Fix", "Implement", "Review", "Deploy"])} #{Enum.random(["API endpoint", "user interface", "database schema", "security patch", "documentation"])}",
...
todos =
  todos ++
    Enum.map(1..20, fn i ->
      ...
      %{
        title:
          "Completed: #{Enum.random(["Built", "Fixed", "Deployed", "Tested", "Documented"])} #{Enum.random(["feature", "module", "service", "component", "integration"])} for #{Enum.random(["users", "admins", "API", "mobile app", "dashboard"])}",
        description:
          "Successfully completed task that improved system performance by #{:rand.uniform(50)}%",
        status: :completed,
...
todos =
  todos ++
    Enum.map(1..15, fn i ->
      ...
      %{
        title:
          "WIP: #{Enum.random(["Developing", "Refactoring", "Optimizing", "Migrating", "Integrating"])} #{Enum.random(["authentication system", "payment gateway", "search functionality", "reporting module", "notification service"])}",
        description:
          "Currently #{progress_percent}% complete. #{Enum.random(["Making good progress", "Some blockers encountered", "On track for deadline", "Ahead of schedule"])}",
        status: :in_progress,
...
todos =
  todos ++
    Enum.map(1..25, fn i ->
      ...
      %{
        title:
          "Future: #{Enum.random(["Plan", "Design", "Implement", "Research", "Evaluate"])} #{Enum.random(["new feature", "architecture", "integration", "migration", "optimization"])}",
        description:
          "Scheduled for future sprint. Requirements: #{Enum.random(["fully defined", "needs refinement", "waiting for dependencies", "approved by stakeholders"])}",
        status: :pending,
...
todos =
  todos ++
    Enum.flat_map(recurring_types, fn {title_prefix, interval, hours} ->
      Enum.map(1..2, fn i ->
        %{
          title:
            "#{title_prefix} - #{Enum.random(["Team A", "Team B", "Platform", "Mobile", "Backend"])}",
...
todos =
  todos ++
    Enum.map(1..10, fn i ->
      %{
        title:
          "Archived: Legacy #{Enum.random(["system", "feature", "module", "service"])} #{Enum.random(["migration", "deprecation", "removal", "update"])}",
...
todos =
  todos ++
    Enum.map(1..10, fn i ->
      %{
        title:
          "Complex: #{Enum.random(["Architect", "Redesign", "Rewrite", "Scale", "Optimize"])} #{Enum.random(["distributed system", "data pipeline", "microservices", "infrastructure", "algorithm"])}",
        description:
          "High complexity task requiring senior developer expertise and careful planning",
        status: Enum.random([:pending, :in_progress]),
...
todos =
  todos ++
    Enum.map(1..15, fn i ->
      ...
      %{
        title:
          "Bug: #{severity} - #{Enum.random(["Memory leak", "UI glitch", "API error", "Data inconsistency", "Performance issue"])} in #{Enum.random(["production", "staging", "development"])}",
        description:
          "Bug report #{1000 + i}. Steps to reproduce included. Affects #{:rand.uniform(1000)} users.",
        ...
        is_recurring: false,
        tags:
          ["bug", severity |> String.downcase()] ++ random_tags.(["backend", "ui", "testing"], 1),
        assigned_to: Enum.random(assignees),
        project: Enum.random(projects),
        complexity:
          case severity do
...
todos =
  todos ++
    Enum.map(1..10, fn i ->
      %{
        title:
          "Feature: #{Enum.random(["Add", "Implement", "Create", "Build"])} #{Enum.random(["dark mode", "export functionality", "real-time updates", "mobile app", "API versioning", "webhooks", "2FA", "audit logs"])}",
        description:
          "User requested feature with #{10 + :rand.uniform(90)} upvotes. Business value: #{Enum.random(["High", "Medium", "Low"])}",
        status: :pending,
